...
describe("UCSBOrganizationIndexPage tests", () => {
    ...
    test("renders three UCSBOrganization correctly for regular user", async () => {
        setupUserOnly();
        axiosMock.onGet("/api/UCSBOrganization/all").reply(200, ucsbOrgFixtures.threeOrgs);
        ...
        const full = screen.getByText("TASA Club");
    ...
    test("what happens when you click delete, admin", async () => {
        ...
        axiosMock.onGet("/api/UCSBOrganization/all").reply(200, ucsbOrgFixtures.threeOrgs);
        axiosMock.onDelete("/api/UCSBOrganization").reply(200, "UCSBOrganization with id ZPR deleted");
        ...
        await waitFor(() => { expect(mockToast).toBeCalledWith("UCSBOrganization with id ZPR deleted") });

        await waitFor(() => { expect(axiosMock.history.delete.length).toBe(1); });
        //console.log(axiosMock.history.delete[0]);
